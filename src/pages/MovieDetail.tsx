...
export default function MovieDetailPage() {
  ...
  const {
    ...
    id ? `https://api.themoviedb.org/3/movie/${id}?language=ko` : null
  );
  ...
  const {
    ...
    id ? `https://api.themoviedb.org/3/movie/${id}/credits?language=ko` : null
  );
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="relative w-full h-[340px] flex items-end overflow-hidden">
        ...
        <div className="relative z-20 w-full max-w-xl p-8">
          ...
          {movie.tagline && (
            <p className="italic mb-2 text-sm text-white/100">{movie.tagline}</p>
          )}
          <p className="text-sm opacity-80 line-clamp-4 whitespace-pre-line mb-1">
            {movie.overview}
          </p>
        </div>
      </div>
        ...
        <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 px-6">
          {directors?.map((person) => (
            <div key={person.id} className="flex flex-col items-center">
              ...
              <div className="text-center text-sm font-bold break-words max-w-[4.5rem]">
                {person.name}
              </div>
              <div className="text-center text-xs text-blue-400 break-words max-w-[4.5rem]">
                감독
              </div>
            </div>
          ))}
          {credits?.cast?.slice(0, 15).map((person) => (
            <div key={person.id} className="flex flex-col items-center">
              ...
              <div className="text-center text-xs text-gray-400 break-words max-w-[4.5rem]">
                {person.character}
              </div>
            </div>
          ))}
