...
export default function MovieDetailPage() {
  ...
  const { movie, credits, isPending, isError } = useFetchMovieDetail(id);
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="w-full px-0">
        ...
        <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-10 gap-6 px-6">
          {/* 감독 */}
          {directors?.map((person) => (
            <div key={person.id} className="flex flex-col items-center">
              ...
              <div className="text-center text-sm font-bold truncate max-w-[10rem]">{person.name}</div>
          ...
          {credits?.cast?.slice(0, 15).map((person) => (
            <div key={person.id} className="flex flex-col items-center">
              ...
              <div className="text-center text-sm font-bold truncate max-w-75rem]">{person.name}</div>
